...
namespace Loxone.Communicator {
	...
	public class WebserviceRequest<T> : WebserviceRequest {
		public WebserviceRequest(WebserviceRequestConfig config, string command, string title = null) : base(config, command, title) {
		}
		...
		public override bool TryValidateResponse(LoxoneResponseMessage response) {
			...
			switch (response.LoxoneFormat) {
				case LoxoneDataFormat.ContentWithControl:
					var requestCommand = this.CommandNotEscaped ?? this.Command;
		...
		public override object Clone() {
			return new WebserviceRequest<T>(this.Config, this.Command) {
